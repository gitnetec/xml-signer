...
export default function Home() {
  ...
  const [signResult, setSignResult] = useState<{ base64GzipXml: string; signedXmlPath: string } | null>(null);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-neutral-950 overflow-hidden">
      ...
      <Card className="relative z-10 w-full max-w-md bg-white/5 backdrop-filter backdrop-blur-sm border border-white/10 rounded-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-white">XML Signature</CardTitle>
        </CardHeader>
        <CardContent>
          <form className="space-y-4" onSubmit={handleFormSubmit}>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="password" className="text-white">PFX File Password</Label>
                <div className="flex items-center space-x-2">
                  <Label htmlFor="password-switch" className="text-white text-sm">
            ...
            <div className="space-y-2">
              <Label htmlFor="xml-text" className="text-white">XML to be signed</Label>
              <Textarea
                id="xml-text"
                value={xmlText}
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <Textarea
                  ...
                  readOnly
                  rows={4}
                  className="mt-1 bg-white/5 backdrop-filter backdrop-blur-sm border border-white/10 text-white"
                />
                <Button onClick={handleCopyBase64} className="mt-2">Copy Base64</Button>
              </div>
              <div>
                <Button onClick={handleDownloadXml}>Download Signed XML</Button>
              </div>
            </div>
          )}
        </CardContent>
